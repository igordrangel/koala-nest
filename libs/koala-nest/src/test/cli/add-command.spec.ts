...
function createMockProject(options: {
  ...
  const appImports = [
    options.crud ? "import { PersonModule } from './controllers/person/person.module';" : "",
    options.health ? "import { HealthCheckModule } from './controllers/health-check/health-check.module';" : "",
    options.auth ? "import { SecurityModule } from './security/security.module';" : "",
  ]
    .filter(Boolean)
    .join("\n");
...
`,
  ...
  if (options.cache) {
    ...
    if (options.cache === "redis") {
      write("src/infra/common/redis-cache.service.ts", "export class RedisCacheService {}\n");
    }
  } else {
  ...
  if (options.cron) {
    write("src/core/utils/cron-expression-to-boolean.ts", "export function cron() {}\n");
  }
