...
When you decorate a new entity with `@Entity`, it is already included in the runtime DataSource — no manual change to `dataSourceFactory` is required. In the migration CLI, `load-all-entities.ts` loads files under `src/domain/entities/` to fill the same `DbContext`.
