...
Migrations ficam em `src/infra/database/migrations/` e são gerenciadas pelos scripts do projeto gerado. Pendentes são aplicadas automaticamente ao subir a API (`dataSourceFactory` → `runMigrations()`).
...
> O script usa `process.execPath` (Node/Bun do ambiente). Com npm ou pnpm, `migration:generate`, `migration:run` e `migration:revert` usam `node --import ts-node/register/transpile-only --require tsconfig-paths/register` para carregar TypeScript e aliases `@/`.
