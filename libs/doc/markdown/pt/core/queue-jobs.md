...
> **Opt-in:** instale com `kl-nest new` (multiselect) ou `kl-nest add queue`. Em **Worker**, costuma ser a feature principal (`kl-nest new my-worker -y --type worker --features queue`). **Não** instala `JobsModule` / `host/jobs` — use `QueueBase` + `IQueueService`.
