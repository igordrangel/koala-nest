...
export function getPrismaModelProp<N extends PrismaModelName>(name: N) {
  return `${name.charAt(0).toLowerCase()}${name.slice(1)}` as PrismaModelProp<N>
}

export function getPrismaDelegate<N extends PrismaModelName>(
  name: N,
  prisma: PrismaClient,
) {
  return prisma[getPrismaModelProp(name)] as PrismaModelDelegate<N>
}
